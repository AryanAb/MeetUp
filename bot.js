...
function eventEmbed(channel, event) {
	...
	if (event.place) {
		maps(event.place.replace(/"/g, '')).then(data => {
			embed.addField('Where?', event.place.replace(/"/g, '') + "\n" + data.candidates[0].formatted_address + "\n" + '[Google Maps](https://www.google.com/maps/search/?api=1&query=' + encodeURI(event.place.replace(/"/g, '')) + " 'Open in Google Maps')", false);
			axios.get('https://maps.googleapis.com/maps/api/place/photo', {
				...
				embed.setImage(response.request.res.responseUrl);
				if (event.ping) {
					embed.addField('Who?', event.ping.replace(/"/g, ''), false);
				}
				if (event.reminder) {
					embed.addField('How much time before reminding?', event.reminder, false);
				}
		...
		if (event.ping) {
			embed.addField('Who?', event.ping, false);
		}
		if (event.reminder) {
			embed.addField('How much time before reminding?', event.reminder, false);
		}
